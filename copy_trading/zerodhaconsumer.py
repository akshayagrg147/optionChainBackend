...
class LiveOptionDataConsumerZerodha(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        try:
            ...
            # Store trading parameters with proper validation
            ...
            self.lot = int(lot) if lot else 75  # Ensure lot is always an integer
    ...
    async def place_zerodha_order(self, transaction_type, trading_symbol, quantity, order_type="MARKET", price=0, exchange=None, product=None, validity=None):
        ...
        try:
            # Use provided exchange or extract from trading symbol (first 3 characters)
            if not exchange:
    ...
    async def execute_reverse_trade(self, PnL):
        ...
        try:
            ...
            # Calculate new quantity based on P&L with proper validation
            ...
            # Calculate quantity with proper error handling
            if self.ltp_at_order and self.ltp_at_order > 0:
                try:
                    # Calculate how many lots we can buy with the available amount
